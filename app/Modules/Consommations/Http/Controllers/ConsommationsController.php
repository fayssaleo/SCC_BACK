...
class ConsommationsController
{
    ...
    {
        $consumptions = json_decode($request->getContent(), true);

        // Extract the IDs from the payload
        $ids = array_column($consumptions, 'id');

        // Log the IDs to be deleted (optional, for debugging purposes)
        Log::info('Deleting consumptions with IDs:', $ids);

        // Perform the deletion
        $deletedCount = Consommations::whereIn('id', $ids)->delete();

        // Respond with the number of records deleted
        return response()->json([
            'message' => 'Consumptions deleted successfully.',
    ...
    public function store_2(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'anomalies.*.date' => 'required',
            'anomalies.*.heure' => 'required|date_format:H:i',
            'anomalies.*.description' => 'nullable|string',
            'anomalies.*.qte' => 'required',
            'anomalies.*.mt_sub' => 'required|numeric',
            'anomalies.*.anomalie' => 'nullable',
            'anomalies.*.nomPrenom' => 'nullable',
        ]);

        // Log validated data
        Log::info('Validated data:', $data);

        // Retrieve all matricules and their corresponding consumers
        ...
        foreach ($data['anomalies'] as $anomaly) {
            ...
            //dd($consommateurs[$anomaly['matricule']]);
            if (!$consommateurId) {
                // If the consommateur does not exist, create it first
                try{
                    $newConsommateur = Consommateurs::create([
                        'username' => $anomaly['matricule'],
                        'firstname' => $anomaly['nomPrenom'],
                        // Add other necessary fields here
                    ]);
                }
                catch(Exception $e ){
                    Log::info('Created new consommateur with ID: ' . $anomaly['matricule']);
                }
                array_push($consommateurs,[$anomaly['matricule'] => $newConsommateur->id ]);
                $consommateurId = $newConsommateur->id;
                Log::info('Created new consommateur with ID: ' . $consommateurId);
            }
            ...
            if (!$exists) {
                $consommationsToInsert[] = [
                    ...
                    'date' => $date,
                    'heure' => $anomaly['heure'],
                    'anomalie' => $anomaly['anomalie'] == 'valid' ? false : true,
                    'absence' => false,
                    'qte' => (int) $anomaly['qte'],
                ...
                // Log or handle existing consumption case
                Log::warning('Existing consumption for matricule: ' . $anomaly['matricule'] . ' on date: ' . $anomaly['date']);
            }
        }

        // Insert the new consumptions in bulk
        if (!empty($consommationsToInsert)) {
            Consommations::insert($consommationsToInsert);
        }

        return response()->json(['message' => 'Anomalies processed successfully.']);
    }
        ...
        try {
            ...
            for ($i = 1; $i < 13; $i++) {
                $anomaliesCount = Consommations::whereYear('date', $year)
                    ...
                    ->where('anomalie', true)
                    ->count();
                $anomaliesCount_2 = Consommations::whereYear('date', $year)
                    ->whereMonth('date', $i)
                    ->count();
                    if($i==7){
                    //    dd($anomaliesCount);
        ...
        $consumption = Consommations::with('consommateur')
            ...
            ->map(function ($item) {
                return [
                    ...
                    'qte' => $item->qte,
                    'mt_sub' => $item->mt_sub,
                    'anomalie' => $item->anomalie?'invalid':'valid'
                ];
            });
