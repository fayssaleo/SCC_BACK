...
class ConsommateursController
{
    ...
    public function setCheckAll(Request $request)
    {
        try {
            $consummers=[];
            for ($i=0; $i < count($request->all()); $i++) {
                $consommateur = Consommateurs::findOrFail($request->all()[$i]["id"]);
                //dd($consommateur);
                $consommateur->checked=$request->all()[$i]["checked"];
                $consommateur->save();
                array_push($consummers,$consommateur);
            }
        } catch (\Throwable $th) {
        ...
        return response()->json($consummers, 200);
    }
